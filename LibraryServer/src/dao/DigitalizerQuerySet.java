...
public class DigitalizerQuerySet { // COMPLETATA E VERIFICATA DEF
	...
	public static UUIDPage createPage(Integer number, Image img, UUIDDocument doc) throws DatabaseException {
		Connection con = null;
		...
		try {
			...
			ps = con.prepareStatement("insert into page(number,image,ID_document) values(?,?,?);",
					new String[] { "ID" });
			ps.setInt(1, number);
			ps.setString(2, img.getUrl());
			ps.setInt(3, doc.getValue());
